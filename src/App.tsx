...
const SERVER_URL =
  import.meta.env.VITE_API_URL ?? "https://bonfire-partake-city.ngrok-free.dev";
...
function App() {
  ...
  const handleFrame = useCallback((nextFrame: PoseFrame) => {
    ...
    if (
      ...
      setHistory((prev) => [
        {
          id: Date.now(),
          label: nextFrame.predictionAr ?? nextFrame.predictionEn ?? '',
  ...
  const selectPrayer = useCallback(async (prayer: PrayerName) => {
    ...
    try {
      ...
      setPrayerError('تعذر اختيار الصلاة. تأكد أن خادم GPU يعمل ثم حاول مرة أخرى.');
    } finally {
  ...
  const sortedProbabilities = frame?.detected
    ? Object.entries(frame.probabilities).sort((a, b) => b[1] - a[1])
    : [];
  const tracking = frame?.tracking ?? null;
  const currentStepName =
    tracking?.expected_pose
      ? STEP_NAMES[tracking.expected_pose] ?? tracking.expected_pose
      : null;
  const isPrayerActive = Boolean(selectedPrayer && tracking?.active);
  const progress =
    tracking?.step_index !== undefined &&
    tracking?.total_steps
      ? Math.min(
          100,
          Math.round((tracking.step_index / tracking.total_steps) * 100),
        )
      : 0;
  return (
    <div className="stage" dir="rtl">
      ...
      <div className="prayer-selector">
        <div className="prayer-selector-title">اختر الصلاة</div>
          ...
          {PRAYERS.map((prayer) => (
            <button
              ...
              className={`prayer-option${
                selectedPrayer === prayer.value ? ' selected' : ''
              }`}
        ...
        {prayerError && (
          <div className="prayer-error" role="alert">
            <AlertTrianimport { useCallback, useEffect, useRef, useState } from 'react';
import { AlertTriangle, RefreshCcw } from 'lucide-react';
import { CameraView } from './CameraView';
import {
  CameraAccessError,
...
function App() {
  ...
  return (
    <div className="stage" dir="rtl">
      ...
      <button
        type="button"
        className={`primary-btn${stream ? ' stop-mode' : ''}`}
      ...
      {error && (
        <div className="error-banner" role="alert">
          ...
          <button type="button" className="retry-btn" onClick={startCamera} disabled={loading}>
            إعادة المحاولة
          </button>
        </div>
      )}

      <div className={`result-sheet${stream && frame?.detected ? ' show' : ''}`}>
        <div className="result-top">
          <span className="result-eyebrow">الوضعية المكتشفة</span>
          <span className="result-conf-badge">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
              <path d="M20 6 9 17l-5-5" />
            </svg>
            {frame?.detected ? `${Math.round(frame.classConfidence * 100)}%` : '—'}
          </span>
        </div>
        <div className="result-name">{frame?.predictionAr ?? ''}</div>
        <div className="prob-list">
          {sortedProbabilities.map(([label, value]) => (
            <div
              key={label}
              className={`prob-row${label === frame?.predictionAr ? ' is-top' : ''}`}
            >
              ...
              <span className="prob-value">{Math.round(value * 100)}%</span>
            </div>
          ))}
        </div>
      </div>

      <div className={`history-section${stream ? ' show' : ''}`}>
        ...
        <div className="history-list">
          {history.length === 0 ? (
            <div className="history-empty">لا يوجد كشف بعد</div>
          ) : (
            history.map((entry) => (
              <div className="history-item" key={entry.id}>
                ...
                <div className="history-body">
                  <div className="history-line1">
                    <span className="name">تم كشف: {entry.label}</span>
                    <span className="conf">{entry.conf}%</span>
                  </div>
                  <div className="history-line2">
                    تم حفظ السجل — {formatTime(entry.time)}
                  </div>
                </div>
              </div>
            ))
